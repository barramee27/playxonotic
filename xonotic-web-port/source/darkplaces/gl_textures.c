...
static textypeinfo_t textype_palette                     = {"palette",                  TEXTYPE_PALETTE       ,  1,  4,  4.0f, GL_RGBA                               , GL_RGBA           , GL_UNSIGNED_BYTE };
static textypeinfo_t textype_palette_alpha               = {"palette_alpha",            TEXTYPE_PALETTE       ,  1,  4,  4.0f, GL_RGBA                               , GL_RGBA           , GL_UNSIGNED_BYTE };
...
// WebGL/GLES2 does not support GL_BGRA as upload format; use GL_RGBA instead.
// The engine's BGRA data will be byte-swapped to RGBA in the upload path.
static textypeinfo_t textype_bgra                        = {"bgra",                     TEXTYPE_BGRA          ,  4,  4,  4.0f, GL_RGBA                               , GL_RGBA           , GL_UNSIGNED_BYTE };
static textypeinfo_t textype_bgra_alpha                  = {"bgra_alpha",               TEXTYPE_BGRA          ,  4,  4,  4.0f, GL_RGBA                               , GL_RGBA           , GL_UNSIGNED_BYTE };
#ifdef __ANDROID__
...
// Swap BGRA byte order to RGBA in-place (for WebGL/GLES2 which lacks GL_BGRA upload)
#ifdef USE_GLES2
static void R_SwapBGRAtoRGBA(unsigned char *data, int numpixels)
{
	int i;
	unsigned char tmp;
	for (i = 0; i < numpixels; i++, data += 4)
	{
		tmp = data[0];
		data[0] = data[2];
		data[2] = tmp;
...
static void R_UploadFullTexture(gltexture_t *glt, const unsigned char *data)
{
	...
	// For non-2D textures (cubemaps, 3D), whole-buffer resize would corrupt
	// the per-face/per-slice data layout.  The cubemap upload path handles
	// per-face resizing itself, so we skip the bulk resize here and only
	// set dimensions.  (Original code Sys_Error'd here, fatal in WebGL.)
	...
	{
		Con_DPrintf("R_UploadFullTexture \"%s\": non-2D texture needs resize (%dx%d -> %dx%d), deferring to per-face upload\n",
			glt->identifier, glt->inputwidth, glt->inputheight, glt->tilewidth, glt->tileheight);
		// Set dimensions to input size; the per-face upload code will resize each face individually
		...
		if (prevbuffer != NULL)
		{
...
#ifdef USE_GLES2
				R_SwapBGRAtoRGBA(colorconvertbuffer, glt->inputwidth * glt->inputheight * glt->inputdepth * glt->sides);
#endif
			...
			if (glt->flags & TEXF_RGBMULTIPLYBYALPHA)
			{
				int alpha;
				R_MakeResizeBufferBigger(width * height * depth * glt->sides * glt->bytesperpixel);
				for (i = 0;i < glt->inputwidth*glt->inputheight*glt->inputdepth*glt->sides*4;i += 4)
				{
...
//		prevbuffer = resizebuffer;
	...
	else
	{
		if (glt->textype->textype == TEXTYPE_PALETTE)
		{
			...
			Image_Copy8bitBGRA(prevbuffer, colorconvertbuffer, glt->inputwidth * glt->inputheight * glt->inputdepth * glt->sides, glt->palette);
#ifdef USE_GLES2
			// WebGL/GLES2 doesn't support GL_BGRA upload format, swap to RGBA
			R_SwapBGRAtoRGBA(colorconvertbuffer, glt->inputwidth * glt->inputheight * glt->inputdepth * glt->sides);
#endif
			prevbuffer = colorconvertbuffer;
		}
	...
	} // end else (2D texture path)

#ifdef USE_GLES2
	// WebGL/GLES2: if the source texture type is BGRA but we're uploading as RGBA,
	// swap the byte order in the buffer. This handles non-palette BGRA textures.
	if (prevbuffer != NULL && glt->textype->textype == TEXTYPE_BGRA)
	{
		int numpixels = width * height * depth * glt->sides;
		// We need a mutable copy since prevbuffer may point to read-only data
		R_MakeResizeBufferBigger(numpixels * glt->bytesperpixel);
		if (prevbuffer != colorconvertbuffer)
		{
			memcpy(colorconvertbuffer, prevbuffer, numpixels * 4);
			prevbuffer = colorconvertbuffer;
		}
		R_SwapBGRAtoRGBA(colorconvertbuffer, numpixels);
	}
#endif
