...
// Ring buffer for incoming WebSocket packets
#define WS_RECV_QUEUE_SIZE 64
#define WS_RECV_PACKET_MAX 4096

typedef struct {
	unsigned char data[WS_RECV_PACKET_MAX];
	int length;
} ws_packet_t;

static ws_packet_t ws_recv_queue[WS_RECV_QUEUE_SIZE];
static int ws_recv_queue_head = 0;
static int ws_recv_queue_tail = 0;
static unsigned int ws_recv_dropped_full = 0;
static unsigned int ws_recv_dropped_size = 0;
static char ws_proxy_url[512] = "";
static char ws_proxy_protocol[64] = "binary";
...
static EM_BOOL ws_onmessage(int eventType, const EmscriptenWebSocketMessageEvent *e, void *userData)
{
	...
	next = (ws_recv_queue_head + 1) % WS_RECV_QUEUE_SIZE;
	if (next == ws_recv_queue_tail)
	{
		if ((++ws_recv_dropped_full & 63) == 1)
			Con_DPrintf("WebSocket receive queue full, dropping packets\n");
		return EM_TRUE; // queue full, drop
	}
	if (e->numBytes > WS_RECV_PACKET_MAX)
	{
		if ((++ws_recv_dropped_size & 15) == 1)
			Con_DPrintf("WebSocket packet too large (%d bytes), dropping\n", e->numBytes);
		return EM_TRUE; // too large, drop
	}
	memcpy(ws_recv_queue[ws_recv_queue_head].data, e->data, e->numBytes);
	ws_recv_queue[ws_recv_queue_head].length = e->numBytes;
	ws_recv_queue_head = next;
	return EM_TRUE;
}
...
static EM_BOOL ws_onopen(int eventType, const EmscriptenWebSocketOpenEvent *e, void *userData)
{
	...
	(void)e;
	(void)userData;
	Con_Printf("WebSocket connected to proxy\n");
	return EM_TRUE;
}
...
static EM_BOOL ws_onclose(int eventType, const EmscriptenWebSocketCloseEvent *e, void *userData)
{
	(void)eventType;
	(void)userData;
	Con_Printf("WebSocket closed (code %d)\n", e->code);
	return EM_TRUE;
}

void LHNET_SetWebSocketProxy(const char *url, const char *protocol)
{
	if (url)
		dpsnprintf(ws_proxy_url, sizeof(ws_proxy_url), "%s", url);
	if (protocol)
		dpsnprintf(ws_proxy_protocol, sizeof(ws_proxy_protocol), "%s", protocol);
	Con_Printf("WebSocket proxy set to: %s (protocol: %s)\n", ws_proxy_url, ws_proxy_protocol);
}
...
#endif
#ifdef __EMSCRIPTEN__
	case LHNETADDRESSTYPE_WEBSOCKET:
		memset(address, 0, sizeof(*address));
		address->addresstype = LHNETADDRESSTYPE_WEBSOCKET;
		address->port = port;
		return 1;
#endif
	}
...
int LHNETADDRESS_ToString(const lhnetaddress_t *vaddress, char *string, int stringbuffersize, int includeport)
{
	...
	*string = 0;
	...
	case LHNETADDRESSTYPE_WEBSOCKET:
		if (includeport)
			return dpsnprintf(string, stringbuffersize, "ws:%d", address->port);
		else
		{
			if (stringbuffersize >= 3)
			{
				memcpy(string, "ws", 3);
				return 2;
			}
	...
	if (lhnetsocket)
	{
		...
		case LHNETADDRESSTYPE_WEBSOCKET:
		{
			...
			if (ws_proxy_url[0])
			{
				...
				attr.url = ws_proxy_url;
				attr.protocols = ws_proxy_protocol;
				...
				if (ws > 0)
				{
					...
					lhnetsocket->wssocket = ws;
					ws_recv_queue_head = 0;
					ws_recv_queue_tail = 0;
					ws_recv_dropped_full = 0;
					ws_recv_dropped_size = 0;
					List_Add_Tail(&lhnetsocket->list, &lhnet_socketlist.list);
					return lhnetsocket;
				}
				else
					Con_Printf("LHNET_OpenSocket: WebSocket creation failed (url: %s)\n", ws_proxy_url);
			}
			else
				Con_Printf("LHNET_OpenSocket: No WebSocket proxy URL set (use em_wss command)\n");
			break;
		}
#endif
		default:
...
#ifdef __EMSCRIPTEN__
		else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_WEBSOCKET)
		{
			emscripten_websocket_close(lhnetsocket->wssocket, 1000, "closing");
			emscripten_websocket_delete(lhnetsocket->wssocket);
		}
#endif
		Z_Free(lhnetsocket);
	}
...
int LHNET_Read(lhnetsocket_t *lhnetsocket, void *content, int maxcontentlength, lhnetaddress_t *vaddress)
{
	...
	{
		if (ws_recv_queue_tail != ws_recv_queue_head)
		{
			ws_packet_t *pkt;
			pkt = &ws_recv_queue[ws_recv_queue_tail];
			if (pkt->length <= maxcontentlength)
			{
				memcpy(content, pkt->data, pkt->length);
				value = pkt->length;
				// Set address to WebSocket type so the engine can route replies
				address->addresstype = LHNETADDRESSTYPE_WEBSOCKET;
				address->port = lhnetsocket->address.port;
			}
			else
				value = -1;
			ws_recv_queue_tail = (ws_recv_queue_tail + 1) % WS_RECV_QUEUE_SIZE;
			return value;
		}
...
int LHNET_Write(lhnetsocket_t *lhnetsocket, const void *content, int contentlength, const lhnetaddress_t *vaddress)
{
	...
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_WEBSOCKET)
	{
		...
		res = emscripten_websocket_send_binary(lhnetsocket->wssocket, (void *)content, contentlength);
		if (res == EMSCRIPTEN_RESULT_SUCCESS)
			value = contentlength;
		else
		{
			Con_DPrintf("LHNET_Write: WebSocket send failed (result %d)\n", res);
			value = -1;
		}
	}
#endif
