...
EM_JS(float, js_GetViewportWidth, (void), {
	...
	return Math.max(1, Math.floor((viewport && viewport.width) || window.innerWidth || document.documentElement.clientWidth || 640));
});
EM_JS(float, js_GetViewportHeight, (void), {
	var viewport = window.visualViewport;
	return Math.max(1, Math.floor((viewport && viewport.height) || window.innerHeight || document.documentElement.clientHeight || 480));
});
...
{
	if (Cmd_Argc(cmd) != 3)
		Con_Printf("Usage: em_wss <url> <binary|text>\n");
	else
	{
		if(strcmp(Cmd_Argv(cmd,2),"binary") == 0 || strcmp(Cmd_Argv(cmd,2),"text") == 0)
		{
			LHNET_SetWebSocketProxy(Cmd_Argv(cmd,1), Cmd_Argv(cmd,2));
...
/*
 * Emscripten 3.1.6 with a custom EM_CACHE may be missing the libc
 * component that creates the WASM-level "main" export from the
 * compiler-generated __main_argc_argv symbol.  Emscripten's JS glue
 * checks for "main" in the WASM exports; if absent, it silently
 * skips calling main().
 *
 * We use __attribute__((export_name("main"))) to force the WASM
 * export name to be literally "main", which is what the JS glue expects.
 */
